    ...
    if isinstance(ast, AST.Identifier):
        ...
        if ast.kind == 'edge':
            ...
            return Types.CallExpression(
                bound=None,
                args=[],
                returns=symbols.get_return_type(key),
                edge=symbols.get_edge(key),
                loc=ast.meta,
            )
        ...
        if ast.name == 'filter':
            ...
            if len(ast.children) == 2:
                ...
                expr = Types.CallExpression(
                    ...
                    edge=symbols.get_edge('Object.filter'),
                    loc=ast.meta,
                )
        elif ast.name == 'dot':
            assert len(ast.children) >= 2
            ...
            for child in ast.children[1:]:
                expr = Types.CallExpression(
                    ...
                    returns=expr.returns,
                    edge=symbols.get_edge(f'{expr.returns.ref}.${ast.name}'),
                    loc=ast.meta,
                )
        return expr
    else:
...
def compile_definitions(ast: AST.ModuleDefinitions):
    ...
    symbols.define_edge(model=Number, name='$gt', returns=Number)
